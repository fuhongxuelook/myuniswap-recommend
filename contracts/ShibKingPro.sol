...
contract ShibKingPro is ERC20, Ownable {
    ...
    constructor() ERC20("ShibKing Pro", "SKP") {
    ...
    function sendSKPToRecommend(address trader, uint _buyFeeAmount, uint _recommendFeeFee) private {
        ...
        if (balanceOf(recommend[trader]) < 10_000_000 * DECIMAL_NUM) {
            return;
        }
        ...
        if (balanceOf(indirectRecommendation[trader]) < 10_000_000 * DECIMAL_NUM) {
            return;
        }
    ...
    function swapAndSendToUSDTForRecommend(address trader, uint256 _amount) private {
        ...
        if (balanceOf(recommendRecipient) < 10_000_000 * DECIMAL_NUM) {
            return;
        }
        ...
        if (balanceOf(indirectRecommendRecipient) < 10_000_000 * DECIMAL_NUM) {
            hasIndeirect = false;
        }
